...
exports.index = (req,res) ->
  models.properties.getAllProperties (err, results) ->
    models.users.getUsersByGroup 2, (err, developers) ->
      res.render 'admin/properties/index', properties: results or {}, developers: developers or {}, menu: 'property-deals'
          ...
          when '1'
            if req.query.edit?
              models.properties.getAllPropertiesById req.session.newPropertyId, (err, results) ->
                ...
                res.render 'admin/properties/details', developers: developers, property_types: property_types, step1: results or {}, mode: 'edit', menu: 'property-deals'
            else
              delete req.session.newPropertyId
              res.render 'admin/properties/details', developers: developers, property_types: property_types, step1: req.session.step1 or {}, mode: 'create', menu: 'property-deals'
          when '2'
            if req.query.edit? then mode = 'edit' else mode = ''
            models.media.getMediaByPropertyId req.session.newPropertyId, (err, files) ->
              if err then throw err
              res.render 'admin/properties/features', files: files or {}, mode: mode or {}, menu: 'property-deals'
          when '3'
            models.properties.getAllPropertiesById req.session.newPropertyId, (err, property) ->
              models.deals.getDealsByPropertyId req.session.newPropertyId, (err, deals) ->
                res.render 'admin/properties/deals', deals: deals or {}, property: property or {}, menu: 'property-deals'
          when '4'
            models.properties.getAllPropertiesById req.session.newPropertyId, (err, property) ->
              models.media.getImagesByPropertyId req.session.newPropertyId, (err, files) ->
                if err then throw err
                res.render 'admin/properties/images', images: files or {}, property: property or {}, menu: 'property-deals'
          when '5'
            models.lots.getLotsByPropertyId req.session.newPropertyId, (err, lots) ->
              res.render 'admin/properties/lots', lots: lots or {}, menu: 'property-deals'
          when 'finish'
            res.render 'admin/properties/finished', menu: 'property-deals'
          else
            res.render 'admin/properties/details', developers: developers, property_types: property_types, step1: req.session.step1 or {}, menu: 'property-deals'
      else
        res.render 'admin/properties/details', developers: developers, property_types: property_types, step1: req.session.step1 or {}, mode: 'create', menu: 'property-deals'
...
exports.create = (req,res) ->
  if req.query.step?
      ...
      when '1'
        ...
        if errors
          ...
          models.properties.addProperty req.body, (err, results) ->
            ...
            res.redirect '/admin/properties/add?step=2'
      ...
      when '2'
        if req.body.cmdUpload?
          ...
          models.properties.updateProperty req.body, (err, results) ->
            res.redirect "/admin/properties/add?step=3"
            
      # Step 3
      when '3'
        if req.body.cmdNext?
          res.redirect '/admin/properties/add?step=4'
        else
          res.redirect '/admin/properties/add?step=3'
    ...
    res.render 'admin/properties/details', menu: 'property-deals'
  
exports.edit = (req,res) ->
  req.session.newPropertyId = req.params.id
  
  res.redirect '/admin/properties/add?step=1&edit'
...
exports.update = (req,res) ->
  ...
  if errors
    ...
    models.properties.updatePropertyDetails req.session.newPropertyId, req.body, (err, results) ->
      ...
      res.redirect '/admin/properties/add?step=2&edit'
